							...
							{certificates.map((cert) => (
								...
								>
									<div className="flex items-start justify-between mb-4">
										...
										<span className="text-xs font-bold uppercase text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/40 px-3 py-1 rounded-full">
											{cert.completionSnapshot?.courseLevel}
										</span>
									</div>
									<h3 className="font-black text-slate-900 dark:text-white text-lg mb-2 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
										{cert.completionSnapshot?.courseName}
									</h3>
									<p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
										Completed on{' '}
										{new Date(cert.completionSnapshot?.completedAt || cert.issuedAt).toLocaleDateString('en-US', {
											year: 'numeric',
											month: 'short',
											day: 'numeric',
										})}
									...
									<div className="flex items-center justify-between">
										...
										<span className="text-sm font-black text-amber-600 dark:text-amber-400 group-hover:translate-x-1 transition-transform">
											→
										</span>
									</div>
								</button>
