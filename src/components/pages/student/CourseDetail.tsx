	...
	if (error || !course) {
		return (
			<div className="flex-1 flex items-center justify-center p-4">
				<div className="bg-red-900/20 border border-red-500/30 rounded-2xl p-4 sm:p-6 max-w-md w-full">
					...
					<button
						...
						className="mt-4 w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 sm:py-2.5 rounded-2xl font-bold text-sm transition-colors"
					>
						Back to Courses
					</button>
				</div>
	...
	return (
		<div className="flex-1 overflow-y-auto">
			<div className="p-3 sm:p-6 md:p-8 max-w-full md:max-w-5xl mx-auto space-y-4 sm:space-y-6 md:space-y-8">
				...
				<div className="bg-gradient-to-r from-blue-600 to-cyan-600 dark:from-blue-700 dark:to-cyan-700 rounded-2xl sm:rounded-2xl md:rounded-2xl p-4 sm:p-8 md:p-12 text-white relative overflow-hidden shadow-xl">
					...
					<div className="relative z-10 flex flex-col flex-wrap md:flex-row justify-between items-start gap-4 sm:gap-6 md:gap-8">
						<div className="flex-1 w-full">
							<div className="inline-block px-3 py-1 text-xs font-black uppercase tracking-widest rounded-2xl bg-white/20 mb-3 sm:mb-4 backdrop-blur-sm">
								{course.level}
							</div>
							...
							<button
								...
								className="inline-flex items-center gap-2 bg-white/20 hover:bg-white/30 border border-white/30 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl font-bold text-xs sm:text-sm transition-colors hover:shadow-md min-h-[44px] sm:min-h-auto"
							>
						...
						<div className="bg-slate-900/40 backdrop-blur-md p-4 sm:p-6 rounded-2xl sm:rounded-2xl border border-white/10 w-full md:w-64 shrink-0">
							{isEnrolled && progress ? (
								<>
									...
									{Number(progress.progressPercent) < 100 && (
										<button
											...
											className="w-full mt-4 sm:mt-6 bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-2xl font-black text-xs sm:text-sm transition-colors flex items-center justify-center space-x-2 min-h-[44px]"
										>
								...
								<>
									...
									<button
										...
										disabled={enrolling}
										className="w-full bg-white hover:bg-slate-100 disabled:bg-slate-500 text-blue-600 disabled:text-white px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl font-black text-xs sm:text-sm transition-colors min-h-[44px]"
									>
				...
				{/* Error Message */}
				{error && (
					<div className="bg-red-900/20 border border-red-500/30 rounded-2xl sm:rounded-2xl p-3 sm:p-4">
					...
					<div className="space-y-2 sm:space-y-3">
						{units?.units.map((unit, index) => {
							...
							return (
								<div
									key={unit.id}
									className={`border border-white/20 dark:border-white/10 rounded-2xl sm:rounded-2xl overflow-hidden transition-all bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl shadow-xl shadow-blue-950/5 ${
										isExpanded
								...
								>
									...
									>
										<div className="flex items-center space-x-2 sm:space-x-3 text-left flex-1 min-w-0">
											<div className="flex items-center justify-center w-8 sm:w-10 h-8 sm:h-10 rounded-2xl bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-black shrink-0">
												{status === 'completed' ? (
													<CheckCircle size={20} />
												) : (
													index + 1
									...
									{isExpanded && !isLocked && (
										<div className="border-t border-blue-200/50 dark:border-blue-400/20 p-3 sm:p-4 md:p-6 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl space-y-3 sm:space-y-4">
											...
											{isEnrolled && progressUnit && (
												<>
													<div className="flex items-center justify-between p-3 sm:p-4 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-white/10">
														<div className="flex-1 min-w-0">
													...
													<button 
														...
														className="w-full bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-2xl font-bold text-xs sm:text-sm transition-colors flex items-center justify-center space-x-2 min-h-[44px]">
														<Play size={16} fill="white" />
